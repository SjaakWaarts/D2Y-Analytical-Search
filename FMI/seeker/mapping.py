...
def copy_mapping(es_mapping, extra=None):
    ...
    # ES 6.x mapping = dsl.Mapping(doc_type)
    # ES 7.x mapping = dsl.Mapping()
    mapping = dsl.Mapping()
...
def document_from_index(es_mapping, document_class=ModelIndex, using='default',extra=None):                        
    """
    Returns an instance of ``document_class`` with a ``Meta`` inner class and default ``queryset`` class method.
    """
    return type('%sDoc' % es_mapping['document'], (document_class,), {
        'Meta': type('Meta', (object,), {
            'index': es_mapping['index'],
            'using': using,
            'mapping': copy_mapping(es_mapping, extra=extra),
        }),
        'queryset': classmethod(lambda cls: model_class.objects.all()),
    })
